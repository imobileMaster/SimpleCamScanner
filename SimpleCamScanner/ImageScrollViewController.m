...
- (IBAction)renameAction:(id)sender {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Rename" message:@"Rname file name." preferredStyle:UIAlertControllerStyleAlert]; // 7
    
    UIAlertAction *defaultAction = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:^(UIAlertAction * action) {
        NSString *newFileName = alert.textFields.firstObject.text;
        NSString* srcPath = [NSString stringWithFormat:@"%@/%@", curDirPath, imageFile];
        NSString* toPath = [NSString stringWithFormat:@"%@/%@", curDirPath, newFileName];
        NSLog(@"filename %@", newFileName);
        NSFileManager *defaultFM = [NSFileManager defaultManager];
        if ([defaultFM fileExistsAtPath:toPath]) {
            NSLog(@"cannot change");
        }
        [defaultFM moveItemAtPath:srcPath toPath:toPath error:nil];
    }]; // 8
    
    [alert addAction:defaultAction]; // 9
    
    [alert addTextFieldWithConfigurationHandler:^(UITextField *textField) {
        textField.placeholder = @"Rename a file name";
