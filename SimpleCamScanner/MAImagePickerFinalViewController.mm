...
- (void)viewDidLoad
{
 ...
 //   [self.navigationController setToolbarHidden:YES animated:YES];
  //  self.navigationItem.title = @"Send Email";
    ...
    _sourceImage = [self thumbnailWithImageWithoutScale:_sourceImage size:CGSizeMake(_sourceImage.size.width, _sourceImage.size.height*0.8)];
    ...
    [_finalImageView setFrame:CGRectMake(0, height, self.view.bounds.size.width, self.view.bounds.size.height - (kCameraToolBarHeight + 70 + height))];
...
- (void)comfirmFinishedImage
{
    ...
    NSData* imageData;
     NSString* fileName;
    if ([Utility getTakeType] == 1) { // new photo in the new folder
        templatesPath = [Utility createDirectoryPath];
    }
    else if ([Utility getTakeType] == 2) { // new photo in existing folder
        templatesPath = [Utility getCurrentDirectoryPath];
    }
    else if ([Utility getTakeType] == 3) { // retake a photo
        templatesPath = [Utility getCurrentDirectoryPath];
    }
    
    if (templatesPath) {
       
        if ([Utility getTakeType] == 3) { // retake a photo
            fileName = [Utility getCurrentFileName];
        }
        else
        {
            fileName = [Utility createFileNameUnderDir:[Utility getCurrentDirectory]];
        }
        
        NSString *filePath = [NSString stringWithFormat:@"%@/%@.jpg", templatesPath, fileName];
        ...
        imageData = UIImageJPEGRepresentation(_adjustedImage, 0);
        [imageData writeToFile:filePath atomically:NO];
        
        [Utility savePDFFile:imageData :templatesPath :fileName];
        
        NSLog(@"before: %@\n\n", filePath);
        
        [[NSNotificationCenter defaultCenter] postNotificationName:@"MAIPCSuccessInternal" object:filePath];
    }
    
    [Utility saveTempData:imageData];
    
    
    DocTableViewController *docTableView = [[DocTableViewController alloc] init];
    docTableView.fileName = fileName;
    docTableView.curDirPath = templatesPath;
    docTableView.isZip = @"No";
...
- (void)setupToolbar
{
    ...
    UIBarButtonItem *confirmButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"Message"] style:UIBarButtonItemStylePlain target:self action:@selector(comfirmFinishedImage)];
